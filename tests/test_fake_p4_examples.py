...
def test_fstat_and_changes_like_examples():
    ...
    p4.register_response('fstat', lambda cmd, *a: [{'depotFile': '//depot/file.txt', 'action': 'edit'}])
    res = p4.run('fstat', '//depot/file.txt')
    assert isinstance(res, list)
    assert res[0]['depotFile'].endswith('file.txt')

    # changes returns a list of changelists
    p4.register_response('changes', lambda cmd, *a: [{'change': 101, 'desc': 'fix'}, {'change': 100, 'desc': 'init'}])
